...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    const enriched = storedCart
      .map(({ id, option }) => {
        ...
        const price = selectedOption?.price ? parseInt(selectedOption.price.replace(/[^0-9]/g, '')) : 120000;
        const originalPrice = selectedOption?.originalPrice ? parseInt(selectedOption.originalPrice.replace(/[^0-9]/g, '')) : 250000;

        return {
          id: course.id,
          title: course.title,
          author: course.author,
          image: course.imageList?.[0] || course.image || '/images/default.jpg',
          originalPrice,
          option,
          price,
          desc: course.desc,
  ...
  const handleOptionChange = (id, newOption) => {
    const updatedItems = cartItems.map((item) => {
      if (item.id === id) {
        const newPrice = item.options?.[newOption]?.price ? parseInt(item.options[newOption].price.replace(/[^0-9]/g, '')) : 120000;
        const newOriginal = item.options?.[newOption]?.originalPrice ? parseInt(item.options[newOption].originalPrice.replace(/[^0-9]/g, '')) : 250000;
    ...
    <section style={{ width: '100vw', margin: '70px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', width: '1270px', margin: '0 auto', padding: '40px' }}>
        <div style={{ width: '65%' }}>
          ...
          {cartItems.length === 0 ? (
            <div style={{ padding: '60px', textAlign: 'center', border: '1px solid #ddd', borderRadius: '10px', backgroundColor: '#f9f9f9' }}>
              ...
              <a href="/" style={{ marginTop: '30px', backgroundColor: '#E57373', color: '#fff', padding: '12px 24px', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', display: 'inline-block', textDecoration: 'none' }}>강의 보러 가기</a>
            </div>
          ) : (
            <>
        ...
        <div style={{ width: '25%', border: '1px solid #ccc', borderRadius: '10px', padding: '30px', height: 'fit-content', margin: '120px 0' }}>
          ...
          <p style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '30px' }}>₩{totalSelectedPrice.toLocaleString()}</p>
          <button onClick={handlePayment} disabled={selectedIds.length === 0} style={{ backgroundColor: selectedIds.length === 0 ? '#ccc' : '#E57373', color: '#fff', width: '100%', padding: '15px', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '15px', cursor: selectedIds.length === 0 ? 'not-allowed' : 'pointer' }}>결제 하기</button>
