...
const CourseList = () => {
  ...
  if (categoryFromState) {
    mainCategory = categoryFromState;
    filteredCourses = dummyCourses.filter(course => course.mainCategory === mainCategory);
  } else {
    mainCategory = Object.keys(allCourses).find(
      (key) => main === getMainSlug(key)
    );
    subCategory = allCourses[mainCategory]?.find((item) => item.slug === sub)?.title;

    filteredCourses = dummyCourses.filter(
      (course) =>
  ...
  if (!mainCategory) return <div style={{ padding: '40px' }}>잘못된 카테고리입니다.</div>;

  const mainSlug = getMainSlug(mainCategory);

  return (
    <div style={{ display: 'flex', width: '1350px', padding: '70px 0', margin: '0 auto' }}>
      {/* 사이드 메뉴 */}
      <aside style={{ width: '210px', marginRight: '32px' }}>
        <div style={{
