...
const maskMap = {
  'IT·付語': '/category_box_1.png',
  'IT·デジタル': '/category_box_1.png',
...
const allCourses = {
  'IT·디지털': [
    { title: '프로그래밍', icon: monitor, slug: 'programming' },
    { title: '사진/영상', icon: movie, slug: 'photo-video' },
    { title: '디자인/3D', icon: square_foot, slug: 'design-3d' },
    { title: 'AI·인공지능', icon: smart_toy, slug: 'ai' },
  ],
  '비즈니스·경제': [
    { title: '마케팅 입문', icon: monitor, slug: 'marketing' },
    { title: '경영 전략', icon: movie, slug: 'strategy' },
    { title: '파이낸스 기초', icon: square_foot, slug: 'finance' },
    { title: '창업 전략', icon: smart_toy, slug: 'startup' },
  ],
  '생활·실무': [
    { title: '컴퓨터 활용능력', icon: monitor, slug: 'computer-skills' },
    { title: '자격증 준비', icon: movie, slug: 'license' },
    { title: '실무 문서 작성법', icon: square_foot, slug: 'document-writing' },
    { title: '업무 자동화', icon: smart_toy, slug: 'automation' },
  ],
  '예술·교양': [
    { title: '글쓰기', icon: monitor, slug: 'writing' },
    { title: '캘리그라피', icon: movie, slug: 'calligraphy' },
    { title: '음악', icon: square_foot, slug: 'music' },
    { title: '타로/사주', icon: smart_toy, slug: 'tarot' },
  ],
};
          ...
          {courseItems.map((item) => (
            <Link
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.background = 'linear-gradient(to bottom, #2D6C83 5%, #3F5F85 95%)'; // 어두운 톤
              }}
              onMouseLeave={e => {
                const img = e.currentTarget.querySelector('img');
                if (img) img.style.transform = 'scale(1)';
                e.currentTarget.style.background = 'linear-gradient(to bottom, #42B2BB 2%, #5584B0 95%)'; // 원래대로
              }}
            >
              <img
                src={item.icon}
