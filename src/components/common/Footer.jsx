import { Link } from 'react-router-dom';

const Footer = () => {
  return (
    <footer>
        <div style={{ backgroundColor: '#F0F0F0'}}>

            {/* 윗줄 */}
            <div style={{ width:'1350px', margin: '0 auto', display:'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px' }}>

                {/* 링크 목록 */}
                <div style={{ display:'flex', gap: '35px' }}>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >공지사항</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >이용약관</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' , fontWeight: 'bold'}} >개인정보처리방침</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >오시는 길</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >FAQ</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >문의하기</Link>
                    <Link to="/login" style={{ color: 'black', fontSize: '15px' }} >환불규정</Link>
                </div>

                {/* 소셜 버튼들 (노, 피, 깃) */}
                <div style={{ gridColumn: 2, display: 'flex', justifyContent: 'end', gap: '20px',  }}>
                    <Link to="/login" style={{ display: 'inline-block',}}>
                        <img
                        src="/icons/sns_kakao.svg"
                        alt="카카오톡"
                        style={{
                            width: '28px',
                            height: '28px',
                            display: 'block',
                            borderRadius: '50px',
                        }}
                        />
                    </Link>
                    <Link to="/login" style={{ display: 'inline-block' }}>
                        <img
                        src="/icons/sns_insta.svg"
                        alt="인스타그램"
                        style={{
                            width: '28px',
                            height: '28px',
                            display: 'block',
                            borderRadius: '50px',
                        }}
                        />
                    </Link>
                    <Link to="/login" style={{ display: 'inline-block' }}>
                        <img
                        src="/icons/sns_youtube.svg"
                        alt="유튜브"
                        style={{
                            width: '28px',
                            height: '28px',
                            display: 'block',
                            borderRadius: '50px',
                        }}
                        />
                    </Link>
                </div>
            </div>

            <hr style={{
                width: '100vw',
                border: 'none',
                height: '1px',
                backgroundColor: '#C2CBD3',
                margin: 0,
                }}
            />

            {/* 로고 및 링크 */}
            <div style={{
                width: '1350px',
                display: 'flex',
                padding: '20px 0 50px 0',
                justifyContent: 'space-between',
                alignItems: 'end',
                margin: '0 auto',
                }}>
                <div style={{ gridColumn: 1, display: 'flex', flexDirection:'column', gap: '20px' }}>


                {/* 회사 정보 */}
                <div>
                    <p style={{ fontSize: '13px', margin: '0 0 5px 0', color: 'rgb(155, 155, 155)' }}>
                    회사명: 배우다(주) ｜ 대표: 홍지원, 이주영 ｜ 사업자등록번호 123-45-67890 ｜
                    서울시 영등포구 여의도동 12-3 빌딩 ｜
                    </p>
                    <p style={{ fontSize: '13px', margin: 0, color: 'rgb(155, 155, 155)' }}>
                    대표전화: 070-1234-5678 ｜ 이메일: asd@fghjk.co.kr
                    </p>
                </div>

                {/* 로고 */}
                <h1 style={{ margin: 0 }}>
                    <img src="/public/logo_black.svg" alt="로고" style={{ height:'25px', }}/>
                </h1>
            </div>

            {/* 모바일 앱 스토어 버튼 */}
            <div style={{ gridColumn: 2, display: 'flex', flexDirection: 'column', gap: '10px', }}>
                <a
                href="https://www.apple.com/app-store/"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                    display: 'flex', width: '160px',
                    backgroundColor: 'white', fontSize: '15px', color: 'black', padding: '10px 0', borderRadius: '50px', gap: '8px' }}
                    >
                    <img
                    src="/icons/store_app.svg"
                    alt="앱스토어"
                    style={{
                        width: '26px',
                        height: '26px',
                        display: 'block',
                        borderRadius: '50px',
                        marginLeft: '20px' 
                    }}
                    />
                    App Store
                </a>
                <a
                    href="https://play.google.com/store/apps/details?id=kr.co.devstory.vocat&hl=ko"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                display: 'flex', width: '160px',
                backgroundColor: 'white', fontSize: '15px', color: 'black', padding: '10px 0', borderRadius: '50px', gap: '5px' }}
                >
                <img
                src="/icons/store_google.svg"
                alt="구글 플레이스토어"
                style={{
                    width: '26px',
                    height: '26px',
                    display: 'block',
                    borderRadius: '50px',
                    marginLeft: '20px' 
                }}
                />
                Google Play
                </a>
            </div>

            </div>
        </div>
</footer>

  )
}

export default Footer